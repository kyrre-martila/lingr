...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'relationship_layers' AND column_name = 'trustScore'
  ) AND NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'relationship_layers' AND column_name = 'trust_score'
  ) THEN
    ALTER TABLE "relationship_layers" RENAME COLUMN "trustScore" TO "trust_score";
  END IF;
END $$;
